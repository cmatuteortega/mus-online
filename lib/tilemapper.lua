...
local function getNeighbours(level, levelsByUid, levelsByIid)
  local _neighbours = level.__neighbours
  local neighbours = {}
  for i = 1, #_neighbours do
    local neighbour = _neighbours[i]
    -- Support both old (levelUid) and new (levelIid) formats
    local levelId = neighbour.levelUid or neighbour.levelIid
    local levelName = levelsByUid[levelId] or levelsByIid[levelId]
    neighbours[neighbour.dir] = levelName
  end
  return neighbours
end

local function getLevel(_level, root, options, tilesets, levelsByUid, levelsByIid)
  ...
  level.next = getNeighbours(_level, levelsByUid, levelsByIid)
  return level
end

local function getLevels(_levels, root, options, tilesets, levelsByUid, levelsByIid)
  local levels = {}
  for i = 1, #_levels do
    local _level = _levels[i]
    local level = getLevel(_level, root, options, tilesets, levelsByUid, levelsByIid)
...
function Tilemapper:new(path, options)
  ...
  -- Build lookup tables for both UID and IID (newer LDtk versions use IID)
  local levelsByUid = {}
  local levelsByIid = {}
  for i = 1, #_levels do
    ...
    if level.iid then
      levelsByIid[level.iid] = level.identifier
    end
  end
  local levels = getLevels(_levels, root, options, tilesets, levelsByUid, levelsByIid)
  self.levels = levels
end
